...
// loadCurrentSmartDefaults resolves the config source and names it in any
// failure so a fail-closed value stays attributable to the serving bundle.
...
// applyBurndown reads the burndown repo-exclusion block. `default` sets the
// fleet-wide posture and each `repo` overrides it.
...
// An explicit per-repo value wins, then the block default.
