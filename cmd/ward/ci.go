...
// listActionTasks reads one page of a repo's Forgejo Actions tasks through the
// in-binary `ward ops forgejo tasks list` leaf (ward#92), not hand-rolled HTTP.
func (r *Runner) listActionTasks(ctx context.Context, owner, repo string, limit int) ([]ciTask, error) {
	cl, err := r.hostForgejoClient(ctx)
	if err != nil {
		return nil, err
	}
	out, err := cl.run(ctx, "tasks", "list", owner, repo, "--limit", strconv.Itoa(limit), "--output", "json")
	if err != nil {
		return nil, fmt.Errorf("ci watch: list tasks for %s/%s: %w", owner, repo, err)
	}
	var resp ciTasksResponse
	if err := json.Unmarshal(out, &resp); err != nil {
		return nil, fmt.Errorf("ci watch: parse tasks for %s/%s: %w", owner, repo, err)
	}
