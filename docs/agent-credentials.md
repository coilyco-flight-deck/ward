...
attributing the merge to `coilysiren`, not the bot. `ensure_git_cred_readable`
re-asserts the perms right before the `setpriv` drop, after every root clone, and
fails loud if the agent still cannot read it.

**How this fix propagates (ward#301).** `entrypoint.sh` is **embedded in the ward
binary** (`//go:embed`) and bind-mounted at `/opt/ward`; `docker run --entrypoint
/opt/ward/entrypoint.sh` makes it **override whatever the dev-base image ships**,
which bakes no entrypoint (see `container_compute.go`). So an `entrypoint.sh`
change propagates on a **ward release**, **not** by republishing the dev-base
image. It shipped in `v0.137.0`.
