...
func TestPreflightTailnet(t *testing.T) {
	...
	if err := r.preflightTailnet(ctx, tailnetPlan()); err != nil {
	...
	err := r.preflightTailnet(ctx, tailnetPlan())
	if err == nil {
...
func TestTailnetTowerBadConfigRefIgnored(t *testing.T) {
	...
	t.Setenv(envTowerOllamaPort, "")
	t.Setenv("WARD_SUBSTRATE_SEED", "")
	t.Setenv("WARD_SUBSTRATE_DEST", "")
	t.Setenv("WARD_SUBSTRATE_MANIFEST", "")
	...
	t.Setenv(envTowerOllamaPort, "19091")
	if got := tailnetNetwork(); got != "env-net" {
		t.Errorf("tailnetNetwork() = %q, want env override", got)
	}
	if got := proxyBoxAddr(); got != "env-proxy:9555" {
		t.Errorf("proxyBoxAddr() = %q, want env override", got)
	}
	if got := towerMagicDNS(); got != "env-tower" {
		t.Errorf("towerMagicDNS() = %q, want env override", got)
	}
	if got := towerOllamaPort(); got != "19091" {
		t.Errorf("towerOllamaPort() = %q, want env override", got)
	}
}
