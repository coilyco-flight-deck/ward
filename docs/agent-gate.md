...
against the same homebrew allow-list the upgrade path trusts, so it can't be
PATH-hijacked. With no canonical path (a dev/source build) or a failed exec, it
falls back to the v1: report the upgrade and tell the operator to re-run,
launching nothing stale.
