...
- **`ward agent {engineer,architect,director,advisor} [--driver <name>]`** (public face: **`warded <role> <ref>`**) - a startup-role roster (ward#347): `engineer` implements a ticket (ref carries, `--watch` attaches, freeform files one first; was work/headless/task), `architect` is a **read-only** scoping session (no push/salvage; was explore, ward#293), `director` an autonomous headless-lane loop (was backlog, ward#346), `advisor` answers without code (ref comments, freeform inline; was reply/ask). The writable `sandbox` is gone. `warded` is a thin `ward` symlink for `ward agent` (ward#282); a **bare ref runs `engineer`**, a bare `#N` infers `owner/repo`. `--driver` picks a harness (`claude|codex|qwen|goose`; ward#185); `--repo` grants writable repos; `--host-net`/`--ts-sidecar` reach the tailnet. Off-org refused; `--print` dry-runs; the engineer carry pre-flights (ward#147). `warded roster` lists every role ([agent-roster.md](agent-roster.md); ward#348). See [agent](agent.md).
