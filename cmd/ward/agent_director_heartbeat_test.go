...
// TestRunDirectorLoopBurndownDrainsImmediately covers the opt-in burndown path: the
// queued work dispatches on the first tick, with no startup prompt in front of it.
func TestRunDirectorLoopBurndownDrainsImmediately(t *testing.T) {
	issue := &backlogEntry{Num: 5, Title: "actionable", Tier: "P0", Lane: "headless", State: "queued"}
	f := &fakeDirector{list: []*backlogEntry{issue}}
	cfg := backlogConfig{maxParallel: 2, pollInterval: time.Millisecond, triage: true}
...
// TestRunDirectorLoopNoTriageStillBurndowns confirms --no-triage only controls
// label-writing. The opt-in burndown heartbeat still dispatches queued work.
func TestRunDirectorLoopNoTriageStillBurndowns(t *testing.T) {
	issue := &backlogEntry{Num: 5, Title: "actionable", Tier: "P0", Lane: "headless", State: "queued"}
	f := &fakeDirector{list: []*backlogEntry{issue}}
