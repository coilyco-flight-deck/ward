...
- **`ward-kdl ops eco {native,server,observe} <verb>`** - `execverb` Eco content pipeline over an `ssh`/`pwsh` hop. **native**/**server** carry the guarded promote envelope (snapshot -> apply -> restart -> await -> canary, auto-rollback); **observe** is the **read-only** sibling ([ward#547](https://forgejo.coilysiren.me/coilyco-flight-deck/ward/issues/547)) - `status`/`logs`/`mods`/`configs` (`sealed`) + `read-config` (traversal-guarded `cat`) - the warded director's non-mutating window on the live kai-server. See [observe](ward-kdl/ward-kdl.eco-observe.guardfile.md).
