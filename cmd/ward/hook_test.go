...
func TestPreToolUse_WardRepoBlocksBareMake(t *testing.T) {
	...
	if code != 2 || !strings.Contains(stderr, "ward exec") {
		t.Fatalf("expected `ward exec` hint, got code=%d stderr=%q", code, stderr)
	}
}

func TestPreToolUse_WardRepoDoesNotBlockGh(t *testing.T) {
	// gh has no ward wrapper. Pass through; permissions.deny handles deny.
	cwd := fakeRepo(t, ".ward/ward.yaml")
	stderr, code := runHook(t, map[string]interface{}{
		"tool_name":  "Bash",
...
func TestPreToolUse_NoGuardMarkerDefaultsToWardTable(t *testing.T) {
	cwd := t.TempDir()
	// No marker: ward table applies. make is in it; gh is not.
	stderrMake, codeMake := runHook(t, map[string]interface{}{
		"tool_name":  "Bash",
		"tool_input": map[string]interface{}{"command": "make build"},
		"cwd":        cwd,
	}, nil)
	if codeMake != 2 || !strings.Contains(stderrMake, "ward exec") {
		t.Fatalf("expected ward exec hint for make, got code=%d stderr=%q", codeMake, stderrMake)
	}
	...
	if codeGh != 0 {
		t.Fatalf("expected gh to pass through under ward default, got code=%d stderr=%q", codeGh, stderrGh)
	}
}
...
func TestPathCheck_WardOffCanonicalPathBlocks(t *testing.T) {
	cwd := fakeRepo(t, ".ward/ward.yaml")
	stderr, code := runHookWithLookup(t,
		map[string]interface{}{
			"tool_name":  "Bash",
			"tool_input": map[string]interface{}{"command": "ward exec build"},
			"cwd":        cwd,
		},
		nil,
		staticLookup("/Users/kai/go/bin/ward"),
	)
	if code != 2 {
		t.Fatalf("expected block on off-path ward, got code=%d stderr=%q", code, stderr)
	}
	if !strings.Contains(stderr, "/Users/kai/go/bin/ward") {
		t.Errorf("expected hijack message to name the offending path, got: %s", stderr)
	}
}
