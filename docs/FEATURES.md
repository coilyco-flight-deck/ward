...
- **`ward drive <harness> [args...]`** (public face: **`warded`**) - drive a headless harness (gptme, goose, ...) behind ward's policy + audit boundary; `warded gptme "..."` reads like `firejail` (ward#247). The first bare token is the harness, splitting ward flags (before) from the harness's own (after); `--` forces passthrough (ward#248). See [docs/drive.md](drive.md).
