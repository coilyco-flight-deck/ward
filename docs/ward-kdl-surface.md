...
- **`ward-kdl-{read,write,admin} ops forgejo <verb>`** - three permission-tiered forgejo binaries (ward#240), layered by `inherit` (cli-guard#160) over wildcard `"*"` grants (cli-guard#159): **read** = `get`/`list`, **write** adds `create`/`edit`, **admin** adds `delete`. Each tier is its own binary, so a withheld verb is absent at compile time, not denied at runtime. Distinct from the curated single-binary `ward-kdl ops forgejo` surface above. See [read](ward-kdl.forgejo.read.guardfile.md), [write](ward-kdl.forgejo.write.guardfile.md), [admin](ward-kdl.forgejo.admin.guardfile.md).
