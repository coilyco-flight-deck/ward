...
// The issue types used to come from cli-guard/cli/dispatch. cli-guard parses a
// reference shape only, so ward owns the issue model and forge routing.
...
// scheme, www, .git, and trailing query / fragment text.
var githubIssueRefRE = regexp.MustCompile(
	`(?i)^(?:https?://)?(?:www\.)?github\.com/([A-Za-z0-9._-]+)/([A-Za-z0-9._-]+?)(?:\.git)?(?:/issues/(\d+)|#(\d+))(?:[/?#].*)?$`,
)

// ParseIssueRef resolves every supported reference form. GitHub is matched
// first, then the rest delegates to cli-guard.
func ParseIssueRef(baseURL, s string) (IssueRef, error) {
	...
	// A scheme-less Forgejo URL is still a Forgejo ref. issueref.Parse only
	// matches it once it carries a scheme.
...
// platformOf tags a ref that issueref.Parse accepted. The Forgejo host
// separates a URL ref from the shortform owner/repo#N.
