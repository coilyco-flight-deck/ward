...
- **`ward agent {work,headless,task,reply,ask,sandbox,explore,backlog} [--driver <name>]`** (public face: **`warded <surface> <ref>`**) - the dispatcher: `work`/`headless` take an issue, `task` files one (ward#164), `reply` comments (ward#179), `ask` answers, `sandbox` is unguided (ward#292), `explore` its **read-only** twin (no push/salvage; ward#293), `backlog` an autonomous supervised loop draining a repo's headless lane (ward#346). `warded` is a thin `ward` symlink for `ward agent` (ward#282); a **bare ref runs `headless`** and a bare `#N` infers `owner/repo`. `--driver` picks a harness (`claude|codex|qwen|goose`; ward#185). `--repo` adds writable repos (ward#230); `--host-net`/`--ts-sidecar` join the tailnet route (ward#332/#333). Off-org refused; `--print` dry-runs; runs reserve; `headless` pre-flights + front-loads (ward#147); `--details` steers. See [agent](agent.md), [agent-sandbox](agent-sandbox.md), [agent-explore](agent-explore.md), [agent-backlog](agent-backlog.md).
