...
func installHooksCommand() *cli.Command {
	return &cli.Command{
		Name:  "install-hooks",
		Usage: "Idempotently register the agent-guard PreToolUse hook in .claude/settings.json.",
		Flags: []cli.Flag{
			&cli.BoolFlag{
				Name:  "dry-run",
...
const (
	wantedMatcher = "Bash"
	wantedCommand = "agent-guard hook pre-tool-use"
	wantedType    = "command"
)
...
func runInstallHooks(args installHooksArgs, out *os.File) error {
	...
	if args.check {
		if present {
			_, _ = fmt.Fprintf(out, "agent-guard install-hooks: hook present at %s\n", target)
			return nil
		}
		return cli.Exit(fmt.Sprintf("agent-guard install-hooks: hook not registered at %s", target), 1)
	}

	if present && !args.dryRun {
		_, _ = fmt.Fprintf(out, "agent-guard install-hooks: hook already registered at %s, nothing to do\n", target)
		return nil
	}
	...
	if args.dryRun {
		_, _ = fmt.Fprintf(out, "agent-guard install-hooks: would write to %s:\n", target)
	...
	_, _ = fmt.Fprintf(out, "agent-guard install-hooks: registered hook in %s\n", target)
	return nil
}
