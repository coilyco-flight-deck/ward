...
// probeWindow/dialTimeout/retryDelay are vars so tests can shrink the window.
var (
...
func preLaunch(rc agentsapi.RunCtx, harness, endpoint, model string, modelProbe modelExistenceProbe) error {
	...
	if derr := probe(rc.Ctx, addr); derr != nil {
		return fmt.Errorf("ollama smoke test: %s's ollama endpoint %s (%s) was unreachable after %s - the dispatched container would hang or fail opaquely instead of a clean abort (ward#487, the local-harness analog of claude's auth smoke test). Point %s at a backend reachable from the container. %s=1 bypasses. (last dial error: %w)", harness, endpoint, addr, probeWindow, harness, skipEnv, derr)
	}
