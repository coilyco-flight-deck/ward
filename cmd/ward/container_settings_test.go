...
func TestContainerSettingsPolicy(t *testing.T) {
	if _, err := containerAssets.ReadFile("containerassets/settings.container.json"); err == nil {
		t.Fatal("settings.container.json must not be shipped as a container asset")
	}
