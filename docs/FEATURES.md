...
- **`ward agent {engineer,architect,director,advisor} [--driver <name>]`** (public face: **`warded <role> <ref>`**) - the dispatcher as a startup-role roster (ward#347): `engineer` implements a ticket (ref carries, `--watch` attaches, freeform files one first; was work/headless/task), `architect` is a **read-only** scoping session (no push/salvage; was explore, ward#293), `director` an autonomous headless-lane loop (was backlog, ward#346), `advisor` answers without code (ref comments, freeform inline; was reply/ask). The writable `sandbox` was removed. `warded` is a thin `ward` symlink for `ward agent` (ward#282); a **bare ref runs `engineer`** and a bare `#N` infers `owner/repo`. `--driver` picks a harness (`claude|codex|qwen|goose`; ward#185); `--repo` grants writable repos (ward#230); `--host-net`/`--ts-sidecar` reach the tailnet (ward#332/#333). Off-org refused; `--print` dry-runs; the engineer carry pre-flights (ward#147). See [agent](agent.md) (per-role docs linked there).
