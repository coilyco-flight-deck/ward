...
func TestJailBreaksBrewPrefix(t *testing.T) {
	cases := []struct {
		name      string
		canonical string
		realPath  string
		...
		{
			// Standard linuxbrew: bin/brew symlinks into the nested Homebrew repo,
			// so the realpath's grandparent (.../Homebrew) differs from the prefix.
			name:      "linuxbrew two-level layout",
			canonical: "/home/linuxbrew/.linuxbrew/bin/brew",
			realPath:  "/home/linuxbrew/.linuxbrew/Homebrew/bin/brew",
			want:      true,
		},
		{
			// macOS Apple Silicon: no <prefix>/Homebrew nesting, realpath == canonical.
			name:      "macos single-level layout",
			canonical: "/opt/homebrew/bin/brew",
			realPath:  "/opt/homebrew/bin/brew",
			want:      false,
		},
		{
			// A symlink that stays within the same prefix must not trip the bypass.
			name:      "symlink within same prefix",
			canonical: "/usr/local/bin/brew",
			realPath:  "/usr/local/libexec/brew",
			want:      false,
		},
		{
			name:      "empty realpath is safe",
			canonical: "/home/linuxbrew/.linuxbrew/bin/brew",
			realPath:  "",
			want:      false,
		},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := jailBreaksBrewPrefix(tc.canonical, tc.realPath); got != tc.want {
				t.Errorf("jailBreaksBrewPrefix(%q, %q) = %v, want %v", tc.canonical, tc.realPath, got, tc.want)
			}
		})
	}
}
