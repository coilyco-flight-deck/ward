...
- **`ward agent {engineer,architect,director,advisor} [--driver <name>]`** (public face: **`warded <role> <ref>`**) - a startup-role roster (ward#347): `engineer` implements **detached only** (ward#356), `architect` is a **read-only** scoping session (ward#293), `director` a headless-lane heartbeat (ward#351), `advisor` answers without code. `warded` is a thin `ward` symlink (ward#282); a **bare ref runs `engineer`**, a bare `#N` infers `owner/repo`. `--driver` picks a harness (`claude|codex|qwen|goose`; ward#185); `--repo` grants repos; `--host-net`/`--ts-sidecar` reach the tailnet (ward#349, ward#359). Off-org refused; `--print` dry-runs; the engineer carry pre-flights (ward#147). `warded roster` lists every role (ward#348). See [agent](agent.md).
