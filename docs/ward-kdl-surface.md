...
- **`ward-kdl ops {aws,kubectl} <verb>`** - `execverb` local-CLI passthroughs: **aws** (SSM/S3/EC2 reads) and **kubectl** (reads + apply/scale/rollout; destructive verbs unexposed). See [aws](ward-kdl/ward-kdl.aws.guardfile.md), [kubectl](ward-kdl/ward-kdl.kubectl.guardfile.md).
