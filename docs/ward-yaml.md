...
- **`security:`** - the security policy block. Read by ward (doctor + hook). The loader and the hook tolerate its absence, but as of ward#450 `ward doctor` **fails** when it is missing (it prints `no security: declared` and exits non-zero), so a real repo declares one. See [security](#security).
...
but as of ward#450 `ward doctor` **fails** on a missing block (see
