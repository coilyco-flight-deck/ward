...
func TestPolicyBoundaryCapabilityForRole(t *testing.T) {
	if caps := capabilityForRole(roleDirector); !caps.tailnet {
		t.Errorf("%s capability = %+v, want tailnet from its guardfile set", roleDirector, caps)
	}
	for _, role := range []string{roleEngineer, roleQA, roleSession, "nonexistent"} {
		if caps := capabilityForRole(role); caps.tailnet {
...
func TestPolicyBoundaryGuardfileInSet(t *testing.T) {
	list := fleetconfig.Guardfiles{List: []string{guardfileTailscale}}
	if !guardfileInSet(guardfileTailscale, list) {
		t.Error("flat list should contain the member")
	}
	if guardfileInSet("git.kdl", list) {
		t.Error("flat list must not contain a non-member")
	}
	prefix := fleetconfig.Guardfiles{Prefix: "tailscale"}
	if !guardfileInSet(guardfileTailscale, prefix) {
		t.Error("prefix set should match the tailnet guardfile")
	}
	if guardfileInSet("git.kdl", prefix) {
...
func TestPolicyBoundaryResolveCapability(t *testing.T) {
	if caps := resolveCapability(roleDirector); !caps.tailnet {
		t.Fatalf("director role capability = %+v, want tailnet from its guardfile set", caps)
	}
	if caps := resolveCapability(roleEngineer); caps.tailnet {
		t.Fatalf("engineer role capability = %+v, want least-access (none)", caps)
	}
	if caps := resolveCapabilityWithOptOut(roleDirector, true); caps.tailnet {
