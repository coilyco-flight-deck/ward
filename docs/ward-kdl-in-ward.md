...
- `wrap ward-kdl pkg brew` -> not mounted into `ward` (standalone `ward-kdl` only)
...
git guardfile does not, and `ward` no longer mounts the `pkg brew` surface at
all. The guardfile stays reachable through the standalone `ward-kdl` binary.
Reconciling the standalone guardfile with its Go surfaces (e.g. routing a future
`git clone` repo-allowlist guard through `ward git`) is left to those surfaces'
own follow-ups.
