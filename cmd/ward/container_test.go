...
// TestEntrypointGooseHeadless locks ward#141: entrypoint runs
// `goose run --no-session -t <seed>` (not claude `-p`) and mirrors doctrine.
func TestEntrypointGooseHeadless(t *testing.T) {
	...
	// The goose `run --no-session -t` block precedes claude's `-p` path.
