...
- **`ward agent {work,headless,task,reply,ask} [--driver <name>]`** (public face: **`warded <surface> <ref>`**) - the container entrypoint and the issue-carrying dispatcher: `work`/`headless` take an issue, `task` *files* one (ward#164), `reply` researches one-shot and comments back (ward#179), `ask` answers a freeform question inline. `warded` is a thin `ward` symlink fronting `ward agent` (ward#247, ward#282); a **bare ref runs `headless`** and a bare `#N` / `N` infers `owner/repo` from the cwd's git origin. `--driver` picks the harness (`claude|codex|qwen|goose`, default claude; ward#185). `--repo owner/name` adds extra writable repos (ward#230, ward#280). Off-org refused, `--print` dry-runs; issue runs *reserve* it (2h TTL); `headless` pre-flights (ward#147); `--details` steers; `--new-tab` opens a Warp tab. See [agent](agent.md), [container-multi-repo](container-multi-repo.md), [agent-reply](agent-reply.md), [agent-ask](agent-ask.md).
