...
- **`ward-kdl ops {aws,kubectl,forgejo-key} <verb>`** - `execverb` local-CLI passthroughs: **aws** (SSM/S3/EC2 reads), **kubectl** (reads + `diff` apply-preview + apply/scale/rollout), **forgejo-key** (a sealed single-key reader for the Forgejo token, the non-2FA SSM bypass, ward#386). See [aws](ward-kdl/ward-kdl.aws.guardfile.md), [kubectl](ward-kdl/ward-kdl.kubectl.guardfile.md), [forgejo-key](ward-kdl/ward-kdl.forgejo-key.guardfile.md).
