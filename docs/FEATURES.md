...
- **`ward agent` roles and workflows** - see [agent.md](agent.md),
  ...
  role definitions plus fleet overlays, not a hand-edited role list. The
  workflow model now uses `merge-remote-main`, `pull-request`,
  `pull-request-and-merge`, and `remote-branch-only`. `ward agent list` now
  carries known engineer capacity alongside the live rows, and `ward agent logs`
  surfaces live docker output and, when that stream is empty, the live
