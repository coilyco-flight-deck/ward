---
doc_goal: Let an operator pick the right per-run landing policy - direct-main, pr/pull-requests, pull-requests-and-merge, or patch-only - by trust level and understand exactly what each mode changes in the seed, container env/label, and reaper gate, including the honest first-slice limits like the still-push-capable token.
---
