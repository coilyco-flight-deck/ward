...
func TestAgentRunningEngineerFromInspectIncludesReservation(t *testing.T) {
	...
	if err := writeAgentReservation(path, agentReservation{
		...
		Repo:      ref.Repo,
		Number:    ref.Number,
		Container: "engineer-codex-sample-sim-18",
		Branch:    "issue-18",
	...
	snap := agentDockerInspectContainer{Name: "/engineer-codex-sample-sim-18"}
	snap.Config.Labels = map[string]string{
		labelDriver: string(modeCodex),
		labelRepo:   "sample-sim",
		labelIssue:  "18",
	}
	snap.Config.Env = []string{
		"WARD_TARGET_OWNER=coilyco-gaming",
		"WARD_TARGET_NAME=sample-sim",
		"WARD_TARGET_REPO=coilyco-gaming/sample-sim",
	...
	if row.Container != "engineer-codex-sample-sim-18" {
		t.Fatalf("container = %q", row.Container)
	}
	if row.Role != roleEngineer {
		t.Fatalf("role = %q", row.Role)
	}
	if row.Ref != "coilyco-gaming/sample-sim#18" {
		t.Fatalf("ref = %q", row.Ref)
	}
	if row.Repo != "coilyco-gaming/sample-sim" {
	...
	if !strings.Contains(string(buf), `"engineer-codex-sample-sim-18"`) {
