...
func prWorkflowRecoverReport(ctx context.Context, cl *forgejoClient, role, owner, repo string, index int) (string, error) {
	...
	linkedState := ""
	if ok {
		linkedText = fmt.Sprintf("%s/%s#%d", owner, repo, linked)
		if issue, ierr := cl.GetIssue(ctx, owner, repo, linked); ierr == nil {
			linkedState = strings.ToLower(strings.TrimSpace(issue.State))
		}
	}
	next := prWorkflowRecoverNextAction(strings.ToLower(strings.TrimSpace(pr.State)), merged, ok, linkedState, linkedText)
...
func prWorkflowRecoverNextAction(state string, merged bool, linkedOK bool, linkedState string, linkedText string) string {
	if merged && linkedOK && linkedState == "open" {
		return fmt.Sprintf("the PR is merged, but the carried issue %s is still open; repair the carried issue trailer or close the issue by hand", linkedText)
	}
	switch state {
	case "closed":
		if merged {
