...
from a carry whose docker daemon runs in a LinuxKit VM that is **not** a tailnet node. The
...
- **No keys, no SSM of its own.** Reached by name, a carry mints no node, injects no
...
A sidecar carry backgrounds a **userspace loopback forwarder** (`ward container forward`,
torn down with the carry). It listens on `127.0.0.1:11434` and bridges each connection to
