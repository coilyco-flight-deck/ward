...
ships in the first place. Keep the dispatching host's ward current through the
install channel you already use and do not pass an older `--ward-version` /
`WARD_AGENT_VERSION` without `--allow-ward-downgrade`.
