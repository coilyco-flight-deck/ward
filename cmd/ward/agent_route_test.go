...
func TestRouteIntakeBody(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ward agent claude task", // names the filing surface
...
func TestRouteUnclearComment(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ward agent claude task <owner/repo>", // how to re-dispatch DIRECT
		"ward agent claude headless",          // the by-hand alternative
		"<details>",                           // folds the read away
