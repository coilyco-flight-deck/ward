...
([agent.md](agent.md)); `--agent` is an equal accepted spelling of the same
pick ([ward#660](https://forgejo.coilysiren.me/coilyco-flight-deck/ward/issues/660)). Two families: **cloud** harnesses authenticate to a
...
spelling `--driver` survives one release as a hidden deprecated alias
(`--harness` and `--agent` are the permanent spellings). Running your own Ollama? [agent-local-model.md](agent-local-model.md) is the
