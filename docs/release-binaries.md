...
(ward#454). The `publish-binaries` job in
...
- **`ward-{darwin,linux}-{amd64,arm64}`** - four static binaries, one per
  ...
  forgejo.coilysiren.me (ward#414, ward#442).
...
The formula itself stays build-from-source (ward#116), so `brew` never consumes
...
pages - there is no second build to diverge (ward#438). The GitHub release also
...
- It authenticates with **`GITHUB_MIRROR_PAT`**, the same PAT the mirror uses
  ...
  ward#454 release run.** Unset, the GitHub half skips loudly and the Forgejo
...
- The release is authored by the PAT user, not `github-actions[bot]`, so the
  mirror's author-guarded release scrub leaves it alone (ward#454, by design in
