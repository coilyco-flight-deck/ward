...
- **`ward upgrade`** - audited ward self-update: `brew update` + `brew upgrade coilyco-flight-deck/tap/ward` (`--dry` shows the version diff via `brew outdated`). Bound to ward-the-formula; `ward pkg brew upgrade` is the path for any other formula. Audit row `upgrade`.
