...
coding agents**. Each invocation takes an issue and drives a
...
[container](container.md)) and a reachable authoritative forge for the target
repo. ward itself still downloads its own release and broker from Forgejo.
...
- [agent-workflow.md](agent-workflow.md) - `--workflow direct-to-main|pull-request|pull-request-and-merge|remote-branch-only`, the run's landing policy.
