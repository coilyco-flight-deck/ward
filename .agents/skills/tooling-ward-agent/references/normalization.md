...
**Owner resolution:** the owner is **whatever org the matched registry row names** - repos span three orgs (`coilysiren`, `coilyco-flight-deck`, `coilyco-gaming`), so there is no fixed default owner. A bare `#N` with no owner/repo tokens, run from inside a repo checkout, lets `ward agent` infer `owner/repo` from the cwd's git origin (ward#282). Refuse if a dictated owner names an org outside the three primary orgs - that is the trust boundary `ward agent` enforces.
