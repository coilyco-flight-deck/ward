...
## ward-kdl-write ops forgejo repo search - cross-repo repo finder (GET /repos/search)
...
## ward-kdl-write ops forgejo org-repo list - list the repos an org owns (GET /orgs/{org}/repos). The route survey (ward#92) catalogs each primary org's repos through this leaf; `user-repo list` covers the one primary owner that is a user, not an org.
...
## ward-kdl-write ops forgejo user-repo list - list the repos a user owns (GET /users/{username}/repos). coilysiren is a user, not an org, so the route survey reaches its repos here rather than through `org-repo list`. Read-only, the survey's other half.
...
## ward-kdl-write ops forgejo user search - user finder (GET /users/search)
...
## ward-kdl-write ops forgejo topic search - topic finder (GET /topics/search)
...
## ward-kdl-write ops forgejo org-label get
...
## ward-kdl-write ops forgejo org-label list
...
## ward-kdl-write ops forgejo org-member list - org member list (GET /orgs/{org}/members)
...
## ward-kdl-write ops forgejo issue search
...
## ward-kdl-write ops forgejo issue-comment list - the comments sub-collection GET, pinned by op (the convention resolves `list issue-comment` against issueGetComments poorly). The `view issue` shadow action below chains this so a view returns the thread.
...
## ward-kdl-write ops forgejo commit list - commit list (GET /repos/{owner}/{repo}/commits). op pinned because the operationId is repoGetAllCommits - a `get`-shaped id the `list commit` convention does not reach.
...
## ward-kdl-write ops forgejo branch list - branch list (GET /repos/{owner}/{repo}/branches). op pinned because a bare `list branch` otherwise resolves to repoListBranchProtection (GET .../branch_protections), the shallower branch-prefixed match.
...
## ward-kdl-write ops forgejo tag list - tag list (GET /repos/{owner}/{repo}/tags)
...
## ward-kdl-write ops forgejo issue-label list
...
## ward-kdl-write ops forgejo tasks list
...
## ward-kdl-write ops forgejo org-label create
...
## ward-kdl-write ops forgejo org-label edit
...
## ward-kdl-write ops forgejo milestone close
...
## ward-kdl-write ops forgejo milestone reopen
...
## ward-kdl-write ops forgejo issue close
...
## ward-kdl-write ops forgejo issue reopen
...
## ward-kdl-write ops forgejo release upload-asset
...
## ward-kdl-write ops forgejo issue-label add
...
## ward-kdl-write ops forgejo issue-label set
...
## ward-kdl-write ops forgejo issue-label remove
...
## ward-kdl-write ops forgejo issue list-all - List all issues by auto-paginating issue list.
...
## ward-kdl-write ops forgejo action move-issue - Move an issue to another repo (copy title/body, back-link, close source). Never deletes.
...
### ward-kdl-write ops forgejo repo fork (denied)

forking is a human-only operation; fork in the web UI

### ward-kdl-write ops forgejo repo archive (denied)

archive/unarchive flips a repo's lifecycle; do it in the web UI

### ward-kdl-write ops forgejo repo unarchive (denied)

archive/unarchive flips a repo's lifecycle; do it in the web UI

### ward-kdl-write ops forgejo org create (denied)

org creation is a human-only operation

### ward-kdl-write ops forgejo org delete (denied)

org deletion is irreversible and human-only

### ward-kdl-write ops forgejo label create (denied)
