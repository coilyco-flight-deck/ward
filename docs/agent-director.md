...
* `~/.ward/config.yaml` provides implicit repo scope. Explicit `--repo` /
  `--org` values override it. Its `config-ref` and `WARD_CONFIG_REF` steer
  generated edge surfaces, not the director control plane. The director does
  not need the current directory to be a git repo.
