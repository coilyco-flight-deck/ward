...
- **`ward agent {engineer,director,advisor} [--driver <name>]`** (public face: **`warded <role> <ref>`**) - a startup-role roster: `engineer` implements **detached**, `director` a heartbeat that **surfaces a read-only session** + **triage**, `advisor` answers without code (freeform interactive; a **ref** posts a comment or **fans cross-repo work into per-repo issues**, trust-gated (ward#424). `warded` is a `ward` symlink; a **bare ref runs `engineer`**, a bare `#N` infers `owner/repo`. `--driver` picks a harness; `--repo`/`--org` scope. See [agent](agent.md).
