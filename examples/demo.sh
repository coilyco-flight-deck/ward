#!/bin/sh
# ward demo - one happy path plus two danger classes (ward#251).
#
...
#
# Safe to run. The happy path runs the toy repo's real test verb. Neither danger
# beat executes anything destructive: the argv-injection is rejected by the gate
# before the command runs, and the protected-binary beat only feeds a string to
# `ward hook pre-tool-use`, which inspects and refuses without executing.
#
# Honest about mechanism (see ../docs/enforcement-boundary.md). Beat 2 is the
# hard gate: the compiled cli-guard verb pipeline every `ward exec` runs through.
# Beat 3 is the Claude PreToolUse hook, a host-side, claude-only, fail-open hint
# - inside a `ward agent` container the hard edge is the container plus cli-guard.
#
...
say "that actually holds. That asymmetry - capability is cheap, denial is the"
say "point - is the whole pitch."
say ""
