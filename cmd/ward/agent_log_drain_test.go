...
// TestResolveSinkMode covers the env override + the local archive default.
func TestResolveSinkMode(t *testing.T) {
	cases := []struct {
		...
		{"", defaultSinkMode},
		{"disk", sinkDisk},
		{"signoz", sinkDisk},         // legacy alias now collapses to disk
		{"both", sinkDisk},           // legacy alias now collapses to disk
	...
	if defaultSinkMode != sinkDisk {
		t.Fatalf("default sink = %q, want local disk", defaultSinkMode)
	}
}
