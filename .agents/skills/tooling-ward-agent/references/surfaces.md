...
`ward agent <role> <ref>` takes a role and `--harness` picks the harness (claude|codex|qwen|goose, default claude; `--agent` is an equal spelling, ward#660). The roster is `engineer`|`director`|`qa` (ward#347, ward#353). This skill dispatches the issue-carrying role, **`engineer`** - it runs the agent in a fresh ephemeral container that clones the repo inside itself. The argument type selects the mode: a ref carries an existing issue (this skill's case); freeform text would file an issue first. (`director` is the autonomous backlog loop that also surfaces a read-only scope + dispatch session on drain - the old `architect` role, folded in by ward#353; `qa` is verdict-only - both out of scope here, this skill takes an already-open ref.)
...
ward agent engineer coilyco-flight-deck/ward#98 --harness codex   # pick another harness
```
