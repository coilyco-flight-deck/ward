...
func TestAgentSeedPromptFrontLoads(t *testing.T) {
	...
	got := agentSeedPrompt(ref, "feat(agent-dispatch): front-load subsystem context",
		"Scan the issue body for ward-kdl, guardfile, ward exec, headless keywords.", "", modeClaude, true)
	...
	plain := agentSeedPrompt(ref, "fix a typo", "just a wording change", "", modeClaude, true)
