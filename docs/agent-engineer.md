...
role of the startup roster: it carries an issue end to end -
...
- **A ref** (`owner/repo#N`, a bare `#N` / `N` inferring `owner/repo` from the cwd's
  git origin, or an issue URL) carries that issue. Namespace policy decides
  whether a bare `owner/repo#N` resolves to GitHub or Forgejo.
